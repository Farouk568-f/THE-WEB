...
const BottomNavbar: React.FC = () => {
    ...
    const navItems = [
      ...
      { to: '/shorts', icon: 'fa-solid fa-bolt', text: t('shorts') },
      { to: '/you', icon: 'fa-solid fa-circle-user', text: t('you'), isCenter: true },
      { to: '/subscriptions', icon: 'fa-solid fa-clapperboard', text: t('subscriptions') },
...
const Layout: React.FC<{ children: React.ReactNode }> = ({ children }) => {
  ...
  const noLayout = location.pathname.startsWith('/shorts');
  ...
  return (
    <div className="min-h-screen text-[var(--text-light)] bg-[var(--background)] transition-colors duration-300">
      { !location.pathname.startsWith('/player') && !isYouPage && <TopNavbar /> }
      <main key={location.pathname} className={`pb-28 ${!noPaddingTop && !isYouPage ? 'pt-20' : ''} animate-page-enter`}>
        {children}
      </main>
