...
const MovieCard: React.FC<{ movie: Movie }> = ({ movie }) => {
  ...
  return (
    ...
    >
      <div className="relative overflow-hidden transition-all duration-300 ease-in-out transform rounded-xl shadow-lg bg-[var(--surface)] interactive-card">
        <img
      ...
      <div className="pt-3">
        ...
        <p className="text-xs text-[var(--text-dark)]">{movie.release_date?.substring(0,4) || movie.first_air_date?.substring(0,4)}</p>
      </div>
    </div>
  );
};
...
const HistoryCard: React.FC<{ item: HistoryItem }> = ({ item }) => {
  ...
  return (
    <div onClick={handleClick} className="relative flex-shrink-0 w-64 overflow-hidden rounded-xl cursor-pointer bg-[var(--surface)] transition-transform duration-300 shadow-lg interactive-card">
      <img src={item.itemImage} alt={item.title} className="object-cover w-full h-36" loading="lazy" />
    ...
    <div>
        ...
        <div className="px-4">
            ...
            <div className="flex pb-4 -mx-4 overflow-x-auto no-scrollbar sm:mx-0">
                <div className="flex flex-nowrap gap-x-5 px-4">
                    {[...Array(5)].map((_, i) => (
                      <div key={i} className="flex-shrink-0 w-36 md:w-40">
                        <div className="w-full h-52 md:h-56 skeleton rounded-xl"></div>
                        <div className="w-3/4 h-4 mt-3 skeleton rounded-md"></div>
                        <div className="w-1/2 h-3 mt-2 skeleton rounded-md"></div>
                      </div>
                    ))}
                </div>
            </div>
...
const ShortsCard: React.FC<{ short: Short; onClick: () => void; }> = ({ short, onClick }) => {
    ...
    return (
        <div onClick={onClick} className="flex-shrink-0 w-28 h-48 md:w-32 md:h-52 cursor-pointer relative overflow-hidden rounded-xl shadow-lg bg-[var(--surface)] interactive-card-sm">
            <img src={`${IMAGE_BASE_URL}${POSTER_SIZE}${short.poster_path}`} alt={short.title || short.name} className="object-cover w-full h-full" loading="lazy" />
  ...
  useEffect(() => {
    ...
    const fetchData = async () => {
      ...
      try {
        ...
        let popularMoviesPromise, topRatedMoviesPromise, seriesPromise, featuredPromise, upcomingPromise, nowPlayingPromise, trendingWeekPromise, popularActorsPromise, movieShortsPromise, tvShortsPromise;
        ...
        const [popularRes, topRatedRes, seriesRes, featuredRes, upcomingRes, nowPlayingRes, trendingWeekRes, popularActorsRes, movieShortsRes, tvShortsRes] = await Promise.all([
        ...
        const [movieShorts, tvShorts] = await Promise.all([
