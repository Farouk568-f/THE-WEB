...
const ResumeCard: React.FC<{ item: HistoryItem }> = ({ item }) => {
    ...
    return (
        <div onClick={handleDetails} className="relative w-full overflow-hidden cursor-pointer group rounded-2xl bg-zinc-800 shadow-xl interactive-card">
                    ...
                    <div className="flex gap-4 px-4">
                        {movies.map((movie) => (
                             <div key={movie.id} onClick={() => navigate(`/details/movie/${movie.id}`)} className="flex-shrink-0 w-32 cursor-pointer group interactive-card-sm">
                                <div className="relative overflow-hidden rounded-2xl bg-zinc-800 shadow-lg">
                                    <img src={`https://image.tmdb.org/t/p/w500${movie.poster_path}`} alt={movie.title} className="w-full aspect-[2/3] object-cover" loading="lazy" />
                                </div>
